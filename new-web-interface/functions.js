...
function parse_config(data){
  ...
  data.split("\n").forEach(function(line){
    ...
    if( key_path.length == 1 ){
      ...
      if( machine.config[key_path[0]] == undefined ){ machine.config[key_path[0]] = {}; }
      machine.config[key_path[0]][key_path[1]] = value;
    }else if(key_path.length == 3 ){
      if( machine.config[key_path[0]] == undefined ){ machine.config[key_path[0]] = {}; }
      if( machine.config[key_path[0]][key_path[1]] == undefined ){ machine.config[key_path[0]][key_path[1]] = {}; }
  ...
  if( machine.config.temperature_control != undefined && ( machine.config.temperature_control.hotend.enable == 'true' || machine.config.temperature_control.bed.enable == 'true' ) ){
    ...
    Object.keys(machine.config.temperature_control).forEach(function(key, index){
      ...
      var min_temp = 20;
      if( key.match(/hotend/) ){ min_temp = 160; }
      var max_temp = 300;
      if( key.match(/bed/) ){ max_temp = 120; }
      if( config.max_temp != undefined ){ max_temp = config.max_temp; }
      var list = [];
      for (i = min_temp; i <= max_temp; i=i+10) { list.push(i); }
      var html = template({name: key, letter: letter, config: config, temps: list });
      $("#temperature_controls").append(html);
      $("#" + letter + "_panel").mouseenter(function(){ $("#" + letter + "_slider").show();
      }).mouseleave(function(){ $("#" + letter + "_slider").hide(); });
      $("#" + letter + "_dial").knob({
        min: min_temp,
        max: max_temp,
  ...
  if( machine.config.root.alpha_max_rate != undefined ){ $("#xy_velocity").val(machine.config.root.alpha_max_rate); }
  if( machine.config.root.gamma_max_rate != undefined ){  $("#z_velocity").val(machine.config.root.gamma_max_rate); }
  ...
  runCommandCallback("M220 M221 M503", function(data){
    var speed = data.split('\n')[0].match(/Speed factor at (.*?) \%/); if( speed ){ data += "\nM220 S" + speed[1]; }
    var flow  = data.split('\n')[1].match(/Flow rate at (.*?) \%/   ); if( flow ){  data += "\nM221 S" + flow[1]; }
    $.each(data.split('\n'), function(index) {
      ...
      if(!codes[0].match(/^\;/)){
        ...
        if(code == 'M301'){ code += 'S' + values['S']; }
        $.each(values, function(index){
          if( machine.config_override[code] === undefined ){ machine.config_override[code] = {}; }
...
function loop_while_playing(){
  ...
  if( machine.playing_counter >= 30 ){ machine.playing_counter=0;}

  // Increase elapsed time
  machine.playing_status.elapsed.second = Number(machine.playing_status.elapsed.second)+1;
  if( machine.playing_status.elapsed.second < 10 ){ machine.playing_status.elapsed.second = '0' + machine.playing_status.elapsed.second; }
  if( machine.playing_status.elapsed.second == 60 ){
    ...
    if( machine.playing_status.elapsed.minute < 10 ){ machine.playing_status.elapsed.minute = '0' + machine.playing_status.elapsed.minute; }
    if( machine.playing_status.elapsed.minute == 60 ){
      ...
      if( machine.playing_status.elapsed.hour < 10 ){ machine.playing_status.elapsed.hour = '0' + machine.playing_status.elapsed.hour; }
    }
  }
...
function getCommandState(cmd, data){
  ...
  if( cmd.match(/^(ls|cd|pwd)/)){ state.icon = 'duplicate'; }
  if( cmd.match(/^help/)       ){ state.icon = 'info-sign'; }
  if( cmd.match(/^cat/)        ){ state.icon = 'file'; }
  if( cmd.match(/^play/)       ){ state.icon = 'play'; }
  if( cmd.match(/^progress/)   ){ state.icon = 'time'; }
  if( cmd.match(/^abort/)      ){ state.icon = 'folder-remove'; state.answer = 'warning'; }
  if( cmd.match(/^mem/)        ){ state.icon = 'hdd'; }
  if( cmd.match(/^(break|dfu)/)){ state.icon = 'scissors'; state.answer = 'warning'; }
  if( cmd.match(/^net/)        ){ state.icon = 'earphone'; }
  if( cmd.match(/^rm/)         ){ state.icon = 'floppy-remove'; state.answer = 'warning'; }
  if( cmd.match(/^suspend/)    ){ state.icon = 'pause'; }
  if( cmd.match(/^resume/)     ){ state.icon = 'play'; }
  if( cmd.match(/^thermistors/)){ state.icon = 'scale'; }
  if( cmd.match(/^fire/)       ){ state.icon = 'fire'; state.answer = 'warning'; }
  if( cmd.match(/^config/)     ){ state.icon = 'th-list'; }
  if( cmd.match(/^remount/)    ){ state.icon = 'repeat'; }
  if( cmd.match(/^reset/)      ){ state.icon = 'repeat'; state.answer = 'warning'; }
  if( cmd.match(/^version/)    ){ state.icon = 'ice-lolly-tasted'; }
  if( cmd.match(/^get/)        ){ state.icon = 'arrow-down'; }
  if( cmd.match(/^set/)        ){ state.icon = 'arrow-up'; }
  if( cmd.match(/^switch/)     ){ state.icon = 'transfer'; }
  if( cmd.match(/^md5sum/)     ){ state.icon = 'stats'; }
...
function jump(){
  ...
  var alpha_min = 0;   if( machine.config.root.alpha_min != undefined){ alpha_min = machine.config.root.alpha_min; }
  var alpha_max = 200; if( machine.config.root.alpha_max != undefined){ alpha_max = machine.config.root.alpha_max; }
  var beta_min  = 0;   if( machine.config.root.beta_min  != undefined){ beta_min  = machine.config.root.beta_min;  }
  var beta_max  = 200; if( machine.config.root.beta_max  != undefined){ beta_max  = machine.config.root.beta_max;  }
  var gamma_min = 0;   if( machine.config.root.gamma_min != undefined){ gamma_min = machine.config.root.gamma_min; }
  var gamma_max = 200; if( machine.config.root.gamma_max != undefined){ gamma_max = machine.config.root.gamma_max; }
